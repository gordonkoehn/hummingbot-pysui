...
from hummingbot.connector.exchange.suidex import (
    suidex_constants as CONSTANTS,
    suidex_utils,
...
class SUIdexExchange(ExchangePyBase):
    ...
    def __init__(
        ...
        suidex_api_key: str,
        suidex_api_secret: str,
        trading_pairs: Optional[List[str]] = None,
        trading_required: bool = True,
        domain: str = CONSTANTS.DEFAULT_DOMAIN,
    ):
        self.api_key = suidex_api_key
        self.secret_key = suidex_api_secret
        self._domain = domain
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._last_trades_poll_suidex_timestamp = 1.0
        super().__init__(client_config_map)

    @staticmethod
    def suidex_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(suidex_type: str) -> OrderType:
        return OrderType[suidex_type]
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(suidex_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _update_order_fills_from_trades(self):
        ...
        if long_interval_current_tick > long_interval_last_tick or (
            ...
            query_time = int(self._last_trades_poll_suidex_timestamp * 1e3)
            self._last_trades_poll_suidex_timestamp = self._time_synchronizer.time()
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(suidex_utils.is_exchange_information_valid, exchange_info["symbols"]):
