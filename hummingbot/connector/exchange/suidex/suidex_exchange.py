...
from hummingbot.connector.exchange.suidex.order_book_data_source import SuidexOrderBookDataSource
...
class SuidexExchange(ExchangePyBase):
    ...
    def _is_request_exception_related_to_time_synchronizer(self, request_exception: Exception) -> bool:
        # Suidex does not use a time synchronizer
        return False
    ...
    async def _user_stream_event_listener(self):
        # Not required in Suidex since all event are processed using the data source PubSub
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return SuidexOrderBookDataSource(
    ...
    def _is_user_stream_initialized(self):
        # Suidex does not have private websocket endpoints
        return self._data_source.is_started()

    def _create_user_stream_tracker(self):
        # Suidex does not use a tracker for the private streams
        return None
    ...
    async def _get_all_pairs_prices(self) -> Dict[str, Any]:
        # Suidex is configured to not be a price provider (check is_price_provider)
